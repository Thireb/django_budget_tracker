...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        with connection.cursor() as cursor:
            ...
            for index in indexes:
                try:
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(f"Dropped index: {index[0]}")
                    )
                except Exception as e:
                    self.stdout.write(
                        self.style.ERROR(f"Error dropping index {index[0]}: {e}")
                    )
        ...
        self.stdout.write(
            self.style.SUCCESS('Successfully cleaned up database tables and indexes')
        ) 